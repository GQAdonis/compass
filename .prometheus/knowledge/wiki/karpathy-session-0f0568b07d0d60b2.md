---
type: SessionRecord
id: karpathy-session-0f0568b07d0d60b2
title: Karpathy session 0f0568b07d0d
tags:
- karpathy
- session-learning
sources:
- session:0cf17349-e0a0-43ed-808c-89b35adbc3e3
timestamp: 2026-09-21T18:30:16.545566+00:00
created_at: 2026-09-21T18:30:16.545566+00:00
updated_at: 2026-09-21T18:30:16.545566+00:00
revision: 0
---

## Delta

Memory indexed. Both monitors are armed — I'll report as results land.

**Where things stand:**

| Item | Status |
|---|---|
| Full workspace wave | **216 suites / 1,730 tests / 0 failures** |
| Every `surrealdb-projection` CI step | verified locally |
| Commits | 5 total, all pushed to `origin/docs/claude-md` |
| CI run 35638432440 | in progress — Windows jobs running on the fixed code |
| Release rebuild | in progress (~16 min) |
| `code-graph-client-platforms (windows-2025)` | **success** |

**Two things still pending, and I won't call the phase done until both resolve:**

1. `surrealdb-projection-windows` — the job that actually proves SurrealDB works on native Windows.
2. Reinstalling the rebuilt binary. The currently-installed `compass` is stale: built before the trail-ranking and degree fixes, so its Surreal `path`/`node_trail` results differ from the repo. I'll reinstall and re-verify end-to-end.

I've also saved build/install facts to project memory — the split `target`/`build-dir`, the three install paths, cold-build timings, and the skill-digest guard that blocks builds after an upstream `SKILL.md` change.
...
- Harness: claude-code
- Session: 0cf17349-e0a0-43ed-808c-89b35adbc3e3
- Captured: 2026-09-21T18:30:14.009028Z
